    ...
    try {
       ...
       await sendEmailFun({
        ...
        return response.status(200).json({
            ...
            token:token, // if needed for verification
        });
...
export async function authWithGoogle(request, response) {
    ...
    try {
        ...
        return response.json({
            message: "Login And Registered successfully",
            error: false,
            success: true,
            data: {
                accesstoken, 
...
export async function updateUserDetails(request, response) {
    ...
    // Optionally, save the code to the user (for future verification)
    userExist.emailVerificationCode = verifyCode;
...
//  reset password without old
export async function resetpasswordNotOld(request,response) {
    try {
...
export async function getReviews(request, response) {
  try {
    ...
    if(!reviews){
      ...
      message: "Review does not added successfully",
      error: true,
      success: false
    });
    }
...
export async function getAllReviews(request,response) {
    try {
        ...
        return response.status(200).json({
         ...
         return response.status(500).json({
      message: error.message || error || "Something is wrong",
      error: true,
      success: false
    });
    }
