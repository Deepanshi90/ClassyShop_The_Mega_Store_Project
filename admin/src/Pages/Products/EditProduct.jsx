...
//             <form className="form p-8 " onSubmit={handleSubmit}>
...
//                                         {isLoading ? <CircularProgress color="inherit" size={24} /> : "Save Changes"}</Button>
//             </form>
...
const EditProduct = () => {
   ...
   const [productRams, setProductRams] = useState([]);
    ...
    useEffect(() =>{
        ...
        fetchDataFromApi(`/api/product/${context?.isOpenFullScreenPanel?.id}`).then((res) =>{
            // console.log(context?.isOpenFullScreenPanel?.id);
            // alert(context?.isOpenFullScreenPanel?.id)
                                    ...
                                    {/* 
{
            ...
            subCat?.children?.map((thirdLevelCat, index) => (
                <MenuItem value={thirdLevelCat?._id} key={index}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2  md:grid-cols-3 lg:grid-cols-4  mb-3 gap-4">
                        ...
                        <div className="col">
                            <h3 className="text-[14px] font-[500] mb-1">Product Rating</h3>
                            <Rating name="half-rating" value={formFields.rating} onChange={onChangeRating} />
                        </div>
  ...
  <h3 className="font-bold text-lg sm:text-xl mb-3">Media and Images</h3>
  ...
  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7 gap-4">
    {previews.length > 0 && previews.map((image, index) => (
      <div className="relative" key={index}>
        ...
        <div className="h-[150px] w-full border border-dashed border-gray-400 bg-gray-100 hover:bg-gray-200 flex items-center justify-center rounded-md overflow-hidden">
          <img src={image} alt="product" className="w-full h-full object-cover" />
        </div>
      </div>
    ))}

    {/* Upload Box (placeholder for new uploads) */}
    <UploadBox
      multiple={true}
      name="images"
