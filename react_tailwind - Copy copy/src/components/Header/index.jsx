    ...
    <header className='bg-white sticky -top-[60px] z-100'>
      ...
      <div className="header py-2 lg:py-4 border-b-[1px] border-gray-200">
        <div className="container flex items-center justify-between">
          ...
          {/* <div className={`col2  fixed top-0 left-0 w-full h-full lg:w-[40%] lg:static p-2 lg:p-0 bg-white z-50
            ${context?.windowWidth>992 && '!block'} ${context?.openSearchPanel === true ? "hidden":  'block' }`}>
            <Search />
          </div> */}
          {context?.windowWidth > 992 ? (
  <div className="col2 static lg:w-[40%] p-2 lg:p-0 bg-white z-50">
    <Search />
  </div>
) : (
  context?.openSearchPanel && (
    <div className="fixed top-0 left-0 w-full h-full bg-white z-50 p-2">
      <Search />
    </div>
  )
)}
            ...
            <ul className='flex items-center justify-end gap-0 lg:gap-3 w-full'>
              {
                context.isLogin === false && context?.windowWidth >992 ?
                  <li className="list-none">
                    <Link to="/login" className='link transition text-[15px] f-[500]'>
                    Login</Link> |  <Link to={"/register"} className='link transition text-[15px] f-[500]'>Register</Link>
                  </li>
                  :
                  (
                    <>
