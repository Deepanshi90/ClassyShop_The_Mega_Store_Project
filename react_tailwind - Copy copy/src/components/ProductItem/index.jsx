    ...
    {isAddedInMyList ? (
      ...
      <FaRegHeart className="text-[18px] text-black group-hover:text-white transition-colors duration-200" />
            ...
            <div className="info p-3 py-5  relative pb-[50px] h-[190px]">
                ...
                <h3 className='text-[12px] lg:text-[14px] title mt-1 font-[500] mb-1 text-[#000]'><Link to={`/product/${props.item?._id}`} className='link transition-all'>
                    {props.item?.name?.substr(0, 30) + '....'}
                </Link></h3>
                <Rating name="size-medium" defaultValue={props.item?.rating} readOnly />
                <br />
                <div className="flex items-center gap-4">
                    <span className='oldPrice line-through text-gray-500 text-[12px] lg:text-[15px] font-[500]'>&#x20b9; {props.item?.oldPrice}</span>
                    <span className='price text-[#ff5252] text-[12px] lg:text-[15px] font-[600]'>&#x20b9; {props.item?.price}</span>
                </div>
                ...
                <div className="!absolute bottom-[15px] left-0 pl-3 w-full">
                    {
                        isAdded === false ?
                            <Button className='btn-org btn-border flex w-full btn-sm gap-2' 
                            onClick={() => addToCart(props?.item, context?.userData?._id, quantity)}>
                                ...
                                {
                                    isLoading === true ?
                                     <Button className='btn-org btn-border  addtocart w-full flex gap-2' size='small'>
